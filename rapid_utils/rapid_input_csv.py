...
def write_connectivity_file(flowline_file,
    ...
    for flowline_id in flowline_id_array:
        ...
        list_upstream_id = flowline_id_array[downstream_id_array == flowline_id]
        ...
        downstream_id = downstream_id_array[flowline_id_array == flowline_id][0]
        #Compile the flowline_id, downstream_id, count_upstream, and
        #list_upstream_id for writing
        list_write.append(np.concatenate([np.array([flowline_id, downstream_id,
                                                    count_upstream]),
                                          list_upstream_id]).astype(int))
    ...
    print('Max count upstream:',max_count_upstream)
...
def write_kfac_file(flowline_file,
    ...
    length_id_field_name : str
        Name of the field containing the length of each flowline (in m or km) in
        the flowline file.
    slope_id_field_name : str
        Name of the field containing the slope of each flowline in the flowline
        file. The slope can be unitless (i.e., m/m or km/km) or represented as a
        percentage (i.e., m/m * 100 or km/km * 100).
    formula_type : int
    ...
    #For Kini1, divide each river length Li by the reference wave celerity C0 of
    #1 km per hour (1000 m per 3600 s):
    if formula_type==1:
        kfac_array=length_array/(1000./3600.) #Length in m divided by (1000 m
                                              #per 3600 s)

    #For Kini2:
    if formula_type==2:
        Kini1_array=length_array/(1000./3600.)
        length_slope_array=length_array/(slope_array**0.5)
        eta_array=np.mean(Kini1)/np.mean(length_slope_array)
    ...
    if formula_type==3:
        ...
        eta_array=np.mean(Kini1_array)/np.mean(length_slope_array)
        kfac_array=eta_array*length_slope_array
